...
class FlighteraFlightsSpider(scrapy.Spider):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.movement_type not in {"departure", "arrival", "all"}:
            raise ValueError(
                "movement_type muss departure, arrival oder all sein!"
            )
        ...
        if self.start_date and self.movement_type != "departure":
            raise ValueError(
                "Der Zeitraum-Modus unterstützt aktuell nur movement_type=departure!"
            )
        ...
        # Abflüge direkt auf der vollständigen Abflugseite laden
        self.start_url = (
        ...
        if self.start_date:
            url_time = self.start_time.replace(":", "_")
            self.start_url += (
                f"/{self.start_date.isoformat()}%20{url_time}"
            )
    ...
    async def start(self):
        if self.start_date:
            callback = self.parse_departure_timeline
        elif self.movement_type == "departure":
            callback = self.parse_current_departures
        else:
            callback = self.parse
    ...
    def parse_current_departures(self, response):
        # Alle Abflüge der aktuellen Seite ausgeben
        items, _ = self.extract_departure_page(response, filter_period=False)
        for item in items:
            yield item

        self.logger.info(
            "%d aktuelle Abflüge für %s gefunden",
            len(items),
            self.airport_icao,
        )
    ...
    def parse(self, response):
        ...
        if not rows:
            self.logger.error(
                "Keine gerenderten Flightera-Tabellen auf %s gefunden",
                response.url,
            )
            return
        ...
        for row in rows:
            ...
            airport_links = cells[3].css('a[href*="/airport/"]')
            if len(airport_links) != 2:
                continue

            origin = self.parse_airport_link(airport_links[0])
            destination = self.parse_airport_link(airport_links[1])
            movement_type = self.identify_movement(origin, destination)
            ...
            deduplication_key = details_url or self.clean_text(
                row.xpath(".//text()").getall()
            )
            if deduplication_key in yielded_urls:
                continue
            yielded_urls.add(deduplication_key)

            scheduled_time, reported_time = self.parse_times(cells[2])
            delay_text = self.extract_delay_text(cells[2])
            status_raw = self.clean_text(cells[0].xpath(".//text()").getall())

            matched_rows += 1
            yield FlightMovementItem(
                observed_at_utc=datetime.now(timezone.utc).isoformat(),
                service_date=self.extract_service_date(details_href),
                ...
                counterpart_iata_code=(
                    destination["iata_code"]
                    if movement_type == "departure"
                    else origin["iata_code"]
                ),
                counterpart_icao_code=(
                    destination["icao_code"]
                    if movement_type == "departure"
                    else origin["icao_code"]
                ),
                ...
                reported_time_local=reported_time,
                delay_minutes=self.parse_delay_minutes(delay_text),
                delay_text=delay_text,
                status=self.normalize_status(status_raw),
                status_raw=status_raw,
                airline_name=self.clean_text(
                    cells[4].css("a::text").getall()
                ),
                aircraft_model=self.clean_text(
                    cells[5].xpath(".//text()").getall()
                ) or None,
        ...
        self.logger.info(
            "%d passende %s-Flugbewegungen für %s gefunden",
            matched_rows,
            self.movement_type,
            self.airport_icao,
        )

    def parse_departure_timeline(self, response):
        if "/verify" in response.url:
            yield from self.retry_verification_page(response)
            return

        self.seen_timeline_urls.add(response.url)
        items, newest_scheduled_datetime = self.extract_departure_page(response)
        for item in items:
            yield item

        self.logger.info(
            "%d Abflüge aus dem Zielzeitraum auf %s gefunden",
            len(items),
            response.url,
        )
    ...
    def extract_departure_page(self, response, filter_period=True):
        rows = response.css("table.flt-responsive-table tbody tr")
        if not rows:
            self.logger.error("Keine Abflugtabelle auf %s gefunden", response.url)
            return [], None
        ...
        for row in rows:
            ...
            destination_links = cells[3].css('a[href*="/airport/"]')
            if not destination_links:
                continue
            destination = self.parse_airport_link(destination_links[0])
            ...
            items.append(FlightMovementItem(
                ...
                airport_icao_code=self.airport_icao,
                counterpart_airport_name=destination["name"],
                counterpart_iata_code=destination["iata_code"],
                counterpart_icao_code=destination["icao_code"],
                origin_airport_name=self.airport_slug,
                origin_icao_code=self.airport_icao,
                destination_airport_name=destination["name"],
                destination_iata_code=destination["iata_code"],
                destination_icao_code=destination["icao_code"],
                flight_number=self.clean_text(
                ...
                reported_time_local=actual_departure,
                delay_minutes=self.parse_delay_minutes(departure_delay_text),
                delay_text=departure_delay_text,
    ...
    def advance_cursor_one_minute(self, href, minutes=1):
        ...
        return (
            f"/en/airport/{self.airport_slug}/{self.airport_icao}/departure"
    ...
    def identify_movement(self, origin, destination):
        if origin["icao_code"] == self.airport_icao:
            return "departure"
        if destination["icao_code"] == self.airport_icao:
            return "arrival"
        return None
    ...
    def parse_times(self, cell):
        scheduled = self.clean_text(cell.css("span.line-through::text").getall())
        reported = self.clean_text(cell.css("span.font-semibold::text").getall())

        if not scheduled:
            return reported or None, None
        return scheduled, reported or None
